...
var uploadCallBack = function(data){
  ...
  if(data.code === 201 || data.code === 200){
    ...
    $("#notification").append('<li><div class="alert-box success"><span>success: </span>'+data.message+'<button class="close">x</button></li>');
  }
  else {
    $("#notification").append('<li><div class="alert-box error"><span>error: </span>'+data.message+'<button class="close">x</button></li>');
  }
...
var AssignmentUploadCallBack = function(data){
  ...
  if(data.code == 201 || data.code == 200)
    $("#notification").html('<div class="alert-box error"><span>error: </span>'+data.message+'</div>');
  else {
    $("#notification").html('<div class="alert-box success"><span>success: </span>'+data.message+'</div>');
  }
  getRepositoryContent();
}
...
function renderRepositoryContent(){
  ...
  for(var i = 0; i < RepoResources.length; i++){
    if($("#"+RepoResources[i].label).length == 0)
      table.append("<tr id='"+RepoResources[i].label+"' class='repo-course-folder' name='"+RepoResources[i].label+"'><td ><i class='material-icons' >arrow_right folder</i></td><td class='folder'><span>"+RepoResources[i].label+"</span></td><td>"+RepoResources[i].ownerId+"</td><td></td></tr>");
  }
